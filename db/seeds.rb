...
# Generate Projects
@skyline = Project.create(name: "Skyline Tower", address: "345 Hammond Way", description: "Super tall modern glass tower, built on a budget too small and schedule too tight",	start_date: "2021-06-25", finish_date: "2022-03-31") 
skyline_users = ["1", "2", "3"]
skyline_users.each do |user|
	@skyline.users << User.find(user)
end		

@flatiron = Project.create(name: "Flatiron Building", address: "136 Smith Ave", description: "Really sweet wedge shaped building that gives an optical illusion when standing at the corner",	start_date: "2021-06-25", finish_date: "2022-12-21") 
flatiron_users = ["1", "2", "3", "5"]
flatiron_users.each do |user|
	@flatiron.users << User.find(user)
end	

@needle = Project.create(name: "Space Needle", address: "25 Worlds Fair Drive", description: "Big tall iconic pointy tower that swings way too much at the top...",	start_date: "2021-07-13", finish_date: "2022-02-15") 
needle_users = ["1", "2", "4", "5"]
needle_users.each do |user|
	@needle.users << User.find(user)
end	

@troll = Project.create(name: "Fremont Troll", address: "9643 Stone Way", description: "Troll sculpture under the fremont bridge.",	start_date: "2021-05-25", finish_date: "2021-08-23") 
troll_users = ["2", "3"]
